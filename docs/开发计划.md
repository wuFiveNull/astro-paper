...
> 本文描述目标架构和实施顺序。列出的 Spring Boot、MySQL、API 与管理功能目前均为计划，不能视为已实现。
>
> 文档更新时间：2026-09-25
...
| `comments` | `post_id`、`user_id`、正文、审核状态、时间戳 | 可加 `parent_id` 支持回复；是否允许游客评论需确认 |
| `messages` | 可空 `user_id`、发件人信息、主题、正文、处理状态、时间戳 | 允许登录用户或游客提交的策略需确认 |
...
| `GET/POST /api/v1/posts/{slug}/comments` | 读取/提交评论 | 读取公开；提交需登录或经确认的游客策略 |
...
2. 正文优先在 `content_markdown` 中保存 Markdown 源文本；服务端渲染后输出内容。
3. 当前 MDX 文章可能引用 Astro/React 组件或构建期资源。此类文章需逐篇识别，改写为普通 Markdown，或明确设计受控的组件白名单。不要把任意 MDX 当作可执行数据库内容。
...
- 确认 Spring Boot 构建工具（建议 Maven）、Java 版本、数据库版本和部署方式。
...
### 阶段 2：公开文章 API 与 Astro SSR

- 实现文章分页、详情、标签、归档和搜索 API。
...
- 本地开发不依赖 Docker。通过 GitHub Actions 运行前端检查/构建和后端编译/测试；当前 CI 仅在 PR 运行，需增加所需的 push/PR 触发规则。
...
- 现有 GitHub CI 只在 Pull Request 和 `workflow_call` 触发；开发后应更新工作流，让前后端构建检查在合适的 push/PR 时执行。
...
以下选择会影响数据库和 API，建议在阶段 0 记录决定：

1. 用户注册开放、邀请制，还是管理员创建账号？
2. 普通访客能否评论/留言，还是都需要登录？
3. 文章正文统一使用 Markdown 吗？图片存服务器还是对象存储？
4. 文章是否需要定时发布、草稿预览、软删除和版本历史？
5. 角色能否给一个用户多个？是否需要作者只能编辑自己的文章？
6. 云服务器已安装哪些运行环境、正式域名和 HTTPS 由谁配置？
