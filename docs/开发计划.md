...
- 2026-09-26 云端 MySQL 互动冒烟测试通过：游客写评论返回 401，普通账号创建待审核评论；管理员发布后公开评论可读；普通账号无法查看后台评论/留言；登录账号提交留言并由管理员处理至 `RESOLVED`。测试卷、容器、随机 `.env` 和上传目录已清理，生产站点/数据库仍运行，可用内存约 768 MiB。
...
- 记录账号、角色、权限变更审计日志。
- 实现文章管理 API 与 Astro 登录/管理/评论/留言交互页面。
