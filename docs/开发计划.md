...
- 本地 `mvn clean verify` 通过；公开文章 API 的 V3 迁移与实际查询仍待云端 MySQL 验证。Astro 页面和数据导入程序还未接入数据库。
...
### 阶段 2：公开文章 API 与 Astro SSR（API 第一版已实现，页面接入未开始）

- [x] 实现文章分页、详情、标签、标签筛选和基础搜索 API。
