...
- HTTP 会话 cookie 使用 HttpOnly、SameSite=Lax，HTTPS 部署通过 `SESSION_COOKIE_SECURE=true` 启用 Secure。账号状态和权限校验读取当前数据库授权信息，已存在的会话在相关操作时会应用最新权限。为云端认证联调新增 API/MySQL 专用低内存 Compose override，服务内存上限合计 576 MiB，不构建 Astro 镜像。
- 新增后端集成验证覆盖管理员初始化、未认证请求拒绝、CSRF 拒绝、登录/退出、账号创建/禁用、权限分配边界、最后管理员保护和普通用户越权拒绝。`mvn clean verify` 当前通过，前端 ESLint 与导入预检通过；认证/RBAC 尚未在云端联调。
- 登录会话目前保存在单个 API 实例内；重启会使会话失效。登录限速、评论/留言写入与审核、文章管理 API、Astro 登录/管理页面仍待完成。
