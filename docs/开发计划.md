...
- 本地 `mvn clean verify` 已通过，健康接口冒烟测试 1 项通过。Flyway SQL 仍需在真实 MySQL 上验证，不能把 H2 冒烟结果当作 MySQL 集成验证。
- 云服务器为 Alibaba Cloud Linux 3；已有 Docker/Compose，现有网站由容器占用 80 端口。服务器宿主机没有 Java、MySQL Server 或 Nginx；磁盘可用空间约 24 GB。
- 云端测试采用独立 Compose 项目：MySQL 不发布宿主机端口，API 只绑定本机回环地址的独立端口，不修改现有网站监听和反向代理。
- 已确认：账号只由管理员创建；评论和留言都要求登录；文章入库使用 Markdown，旧 MDX 在迁移导入时转换处理。
- 当前仅完成后端基础的第一步；MySQL 迁移和容器联调完成前，阶段 1 不标记为完成。
