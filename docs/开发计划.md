...
- 阶段 2 的页面 SSR/API 与导入工具测试已验证；正式数据库仍需先配置首次管理员，再审查并执行文章迁移。阶段 3 的认证/RBAC、评论/留言 API 和首批管理页面现已实现，文章管理与其余后台功能仍待完成。
...
- 登录会话目前保存在单个 API 实例内；重启会使会话失效。登录限速、账号/权限审计日志、文章管理 API 与页面、评论/留言 UI 的浏览器端联调及正式生产部署仍待完成。
...
- 记录账号、角色、权限变更审计日志。
- 实现文章管理 API 与页面，并完成评论/留言 UI 的浏览器端联调。
